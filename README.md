...
### 1. Новый узел включается в систему
...
# 1. Регистрируем узел (он автоматически принадлежит alice)
NODE_ID=$(curl -s -X POST http://localhost:8080/api/v1/nodes \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"name":"api-gateway","type":"gateway"}' | jq -r '.id')

# 2. Узел периодически шлёт метрики (тоже под токеном Alice)
curl -X POST "http://localhost:8080/api/v1/metrics/nodes/$NODE_ID" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"name":"http.requests","value":1,"type":"COUNTER"}'

# Heartbeat отдельно отправлять не нужно — его обновляет приём метрик.
# Если другой пользователь (Bob) сделает GET /api/v1/nodes — он не увидит
# узлов Alice. Доступ к чужому ID вернёт 404.
```

### 2. Получить статистику по CPU за последние 15 минут

```bash
curl "http://localhost:8080/api/v1/metrics/summary?name=cpu.usage&windowSeconds=900"
```

### 3. Настроить алерт на p95 времени ответа
...
### 4. Посмотреть какие узлы сейчас нездоровы

```bash
curl "http://localhost:8080/api/v1/nodes?status=UNHEALTHY"
```

### 5. Посмотреть все сработавшие алерты за сессию
...
В проекте 57 автотестов на JUnit 5. Запуск:
